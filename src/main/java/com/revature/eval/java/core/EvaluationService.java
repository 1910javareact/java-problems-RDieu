...
public class EvaluationService {
	...
	public String reverse(String string) {
		...
		char[] rever = new char[string.length()];
		
		char[] verse = string.toCharArray();
		
		
		for(int i = 0; i < string.length(); ++i) {
		
			rever[string.length() - 1 - i] = verse[i];
		}
		
		String outcome = new String(rever);
...
static class BinarySearch<T> {
	...
	static class RotationalCipher {
		...
		public String rotate(String string) {
			StringBuilder rotational = new StringBuilder();
			...
			for (int i = 0; i < inputString.length; i++) {
				...
				if ((int) inputLetter >= 65 && (int) inputLetter <= 90) {
					...
					if (shiftBy > 90) {
						shiftBy = shiftBy - 90;
						rotational.append((char) ('A' + shiftBy - 1));
					} else {
						rotational.append((char) shiftBy);
					}
					...
					rotational.append(inputLetter);
				}
			}
			return rotational.toString();
		}
	...
	public int calculateNthPrime(int i) 
	{
		...
		if(i < 1){throw new java.lang.IllegalArgumentException();
		}
		List<Integer> NthPrime = new ArrayList<Integer>();
		int count = 1;
		int n;
		
		while(NthPrime.size() < i) {
			count++;
	 ...
	 */
		...
		public int getSumOfMultiples(int i, int[] set) {
			...
			Set<Integer> tiple = new HashSet<Integer>();
	 ...
	 */
		public boolean isLuhnValid(String string) {
			...
			List<Integer> Luhn = new ArrayList<Integer>();
			int result = 0;
			string = string.replaceAll("\\s+", "");
			...
			for(int i = 0; i < string.length(); i++) {
				Luhn.add(Character.getNumericValue(string.charAt(i)));
			}
			for(int j = Luhn.size() - 2; j >= 0; j -= 2) {
				Luhn.set(j, Luhn.get(j) * 2);
				if(Luhn.get(j) > 9) {
					Luhn.set(j, Luhn.get(j) - 9);
				}
			}
			for(int k = 0; k < Luhn.size(); k++) {
				result +=Luhn.get(k);
			}
			if(result%10 == 0) {
				return true;
	 ...
	 */
		public int solveWordProblem(String string) {
			...
			String[] problem = string.split(string);
			...
			for(int j = 0; j < problem.length; j++) {
				if(problem[j].matches("\\s+")) {
					numbers.add(Integer.parseInt(problem[j]));
				}
				else if(problem[j].equals("plus")) {
					evaluate = "+";
				}
				else if(problem[j].equals("minus")) {
					evaluate = "-";
				}
				else if(problem[j].equals("multiplied") || problem[j].equals("times")) {
					evaluate = "*";
				}
				else if(problem[j].equals("divided")) {
					evaluate = "/";
				}
			}
